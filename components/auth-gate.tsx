...
export default function AuthGate({children}:{children:React.ReactNode}){
 ...
 async function submit(action:string,e?:React.SyntheticEvent<HTMLFormElement>){
  ...
  try{
   const r=await apiFetch('/api/auth',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,password,newPassword,remember:action==='login'&&remember}),signal:AbortSignal.timeout(15000)});
 ...
 if(status==='out')return <main className="login-shell"><section className="login-card"><div className="brand-mark"><Star/></div><p className="eyebrow">高老師的班級獎勵簿</p><h1>登入班級系統</h1><p>使用系統密碼即可登入，無需 ChatGPT 帳號。</p>{notice&&<output className="auth-notice">{notice}</output>}<form onSubmit={e=>void submit('login',e)}><label htmlFor="login-password">系統密碼</label><input id="login-password" type="password" autoComplete="current-password" value={password} required maxLength={128} onChange={e=>setPassword(e.target.value)}/><label className="remember-device" htmlFor="remember-device"><input id="remember-device" type="checkbox" checked={remember} onChange={e=>setRemember(e.target.checked)}/><span><strong>記住這台裝置 30 天</strong><small>適合您專用的電腦、手機或平板</small></span></label>{error&&<p className="auth-error" role="alert">{error}</p>}<button className="action primary" disabled={busy} type="submit"><LockKeyhole size={17}/>{busy?'登入中…':'登入'}</button></form><p className="login-footnote">共用裝置請勿勾選；按「登出」會取消這台裝置的記住狀態。</p></section></main>;
